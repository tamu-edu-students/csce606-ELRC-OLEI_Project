...
Given('I try to login') do
  ...
  OmniAuth.config.mock_auth[:auth0] = OmniAuth::AuthHash.new({ provider: 'auth0',
                                                               ...
                                                               credentials: {
                                                                 token: 'eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIiwiaXNzIjoiaHR0cHM6Ly9kZXYtemNmdWNrM3gnN3c1djZzYS51cy5hdXRoMC5jb20vIn0..H0PuCkDtR_V2yeJT.r6XP3TPa2Bho1wmCsE0YdGkIBciyKOPR_3K4iZvDWLmynB5mYfRwr_gJ4ZX0E9aUpRAboJ4R9VpKYySlhIgw6RC_3UhGjEgciJumb_yqbGZJfG3OQ86GEq-iCgJJgnLGB1x-q7OykkFD-NvcL-ezhbfc8khAZJfHypupFEVIneTs9Jdi4Wp5jJdsmQ6Y_-O5pACQcp5WhpQ42-Jz_mbL_f-ogs5rngRjRmmujDNgwzIQvSwYLo6yxc1C71IoDAX1PwbZVImu_jttICuUJV1GnftNvvVwb4CLQCNaEjJ386qoNTRbN6m1MjcyAuPKa3DBKmgNGNSykTD2ZZGtPwfp5Xg.lEoIk0uUzvfB97t9qLxJNQ',
...
Then('I fill in my first and last name, district name, campus name, and organization role, and click create') do
  fill_in 'First name', with: 'John'
  fill_in 'Last name', with: 'Doe'
  fill_in 'Campus name', with: 'Joe Campus'
  fill_in 'District name', with: 'Joe District'

  expect(page).to have_select('Role', options: ["Board Member", "Superintendent", "Assistant/Associate Superintendent", "Principal", "Assistant Principal", "Counselor", "Teacher Leader (Specialist)", "Teacher", "Other Instructional Staff", "Non-Instructional Staff"])
...
Then('a survey profile is created with the proper information') do
  ...
  expect(profile.campus_name).to eq('Joe Campus')
  expect(profile.district_name).to eq('Joe District')
...
Given('I have created a survey profile previously') do
  SurveyProfile.create(user_id: 'google-oauth2|100507718411999601151', first_name: 'John', last_name: 'Doe', campus_name: 'Joe Campus', district_name: 'Joe District', role: 'Principal')
end
